...
const universities = graduates.map((currentItem) => {
...
const contactInfo = graduates.map((currentItem) => {
  return `${currentItem.first_name} ${currentItem.email}`
});
...
zooAnimals.map((currentItem) => {
  lowCaseAnimalNames.push(currentItem.animal_name.toLowerCase())
})
...
*/
const lowPopulationAnimals = zooAnimals.filter((currentItem) => {
...
*/
const populationTotal = zooAnimals.reduce((accumulator, currentItem) => {
